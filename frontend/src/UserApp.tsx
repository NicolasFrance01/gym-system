          ...
          <div className="flex flex-col min-h-0 h-full max-h-[75vh] space-y-4 animate-in slide-in-from-bottom-8 duration-500 overflow-hidden">
             ...
             <div className="flex-1 overflow-y-auto pr-1 space-y-3 custom-scrollbar min-h-0">
                {userData.currentRoutine.map(ex => (
                  <div key={ex.id} className={`p-4 rounded-3xl border transition-all ${ex.completed ? 'bg-green-500/10 border-green-500/20 shadow-lg shadow-green-500/5' : 'bg-[#141b29] border-white/5'} space-y-3`}>
                     <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                           <div onClick={()=>toggleExercise(ex.id)} className={`w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer transition-all ${ex.completed ? 'bg-green-500 text-white shadow-lg shadow-green-500/30' : 'bg-white/5 text-white/20 hover:text-white hover:bg-white/10'}`}>
                        ...
                        <button onClick={()=>toggleExercise(ex.id)} className={`px-3 py-1.5 rounded-lg text-[8px] font-black uppercase ${ex.completed ? 'bg-green-500 text-white' : 'bg-white/5 text-white/40'}`}>{ex.completed ? 'Hecho' : 'Completar'}</button>
                     </div>
                     <div className="flex items-center gap-3 bg-black/40 rounded-2xl p-3 border border-white/5">
                        ...
                        <input type="number" className="bg-transparent text-white font-black text-lg w-12 outline-none text-right" value={ex.weight} onChange={e=>updateWeight(ex.id, parseInt(e.target.value) || 0)} />
