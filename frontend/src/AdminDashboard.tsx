...
export default function AdminDashboard() {
  ...
  const [modalType, setModalType] = useState<'member' | 'staff' | 'workout'>('member');
      ...
      default: return (
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <SummaryCard title="Personal" value={staff.length} icon={<Briefcase size={18}/>} onClick={() => setActiveTab('Staff')} color="purple" />
          </div>
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-white/5 border border-white/5 p-6 rounded-2xl">
              <h3 className="text-sm font-bold mb-4">Ingresos Proyectados</h3>
...
function MembersModule({ members, onEdit, onDelete, onAddClick, onPayClick }: any) {
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center"><h3 className="font-bold text-base">Socios</h3><button onClick={onAddClick} className="bg-blue-600 px-4 py-2 rounded-xl text-[11px] font-bold">+ Nuevo</button></div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
         {members.map((m: any) => (
           <div key={m.id} className="p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-blue-500/20 transition-all">
             <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><div className="w-8 h-8 bg-neutral-800 rounded-lg flex items-center justify-center font-bold">{m.name[0]}</div><p className="font-bold text-white text-xs">{m.name}</p></div>{m.status === 'ACTIVO' ? <CheckCircle className="text-green-500" size={14} /> : <XCircle className="text-red-500" size={14} />}</div>
             <div className="grid grid-cols-2 gap-2 mt-4">
               ...
               <button onClick={() => onDelete(m.id)} className="col-span-2 p-2 bg-red-500/10 text-red-500 rounded-lg text-[9px] font-bold hover:bg-red-500 transition-all">Baja</button>
             </div>
           </div>
         ))}
      </div>
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 bg-white/5 border border-white/5 rounded-2xl p-6">
        <h3 className="font-bold mb-6 text-[10px] text-white/30 uppercase tracking-widest">Ingresos Proyectados</h3>
        <div className="h-60"><ResponsiveContainer width="100%" height="100%"><BarChart data={data.cashflow_data}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} /><XAxis dataKey="month" stroke="rgba(255,255,255,0.2)" fontSize={9} /><YAxis stroke="rgba(255,255,255,0.2)" fontSize={9} /><Tooltip contentStyle={{ backgroundColor: '#111', border: 'none' }} /><Bar dataKey="ingresos" fill="#3b82f6" radius={[3, 3, 0, 0]} /></BarChart></ResponsiveContainer></div>
      </div>
      <div className="bg-white/5 border border-white/5 rounded-2xl p-6">
        <h3 className="font-bold mb-6 text-[10px] text-white/30 uppercase tracking-widest">Mix Comercial</h3>
  ...
  return (
    <div className="space-y-6">
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 border border-white/5 rounded-2xl p-6">
          <h3 className="font-bold mb-6 text-[10px] text-white/30 uppercase tracking-widest flex items-center gap-2"><TrendingUp size={14} /> Retención Proyectada</h3>
          <div className="h-60"><ResponsiveContainer width="100%" height="100%"><AreaChart data={data.member_growth}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" vertical={false} /><XAxis dataKey="month" stroke="rgba(255,255,255,0.2)" fontSize={9} /><Tooltip contentStyle={{ backgroundColor: '#111' }} /><Area type="monotone" dataKey="altas" stroke="#10b981" fill="#10b981" fillOpacity={0.15} isAnimationActive={false} /></AreaChart></ResponsiveContainer></div>
        </div>
      </div>
      <div className="bg-white/5 border border-white/5 rounded-2xl p-6">
